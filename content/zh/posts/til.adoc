...
- muted variables 集合 M 用来暂时容忍某些已经越界、但未来可能被 continuation 重新绑定的变量。
- stack mark I 用来记录“哪些栈帧从未被 effect capture 过”，从而判断何时可以安全地把 muted variables 重新 unmute。

I 记录栈至少要退回到哪里，才能确定已经没有 continuation 能把 muted 变量重新变为安全的。
