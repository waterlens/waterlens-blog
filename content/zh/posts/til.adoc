...
- portable -> contented
- shared | uncontented -> nonportable
...
初始时 I = ⊤，每次 effect 被处理时，I 会更新成 min(len(E1), I)。
所以 I 只会被 raised effects 变小，不会变大。

I 什么时候触发 unmute：当执行退回到某个点，满足当前栈深度 len(E) \<= I，这时无法让 muted variables 恢复，因此：
