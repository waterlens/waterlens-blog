...
对于优化算法 stem:[I], 它接受一个输入表达式 stem:[e]，一个用于确定执行变换目的的语境 (Context) stem:[\gamma]，
一个负责映射变量名的环境 (Environment) stem:[\rho]，
以及延续 stem:[k] 和存储 stem:[\sigma]。延续 stem:[k] 的存在是因为描述算法时使用了 Continuation Passing Style (CPS) ，实践中可以用其他方式代替。
存储 stem:[\sigma] 里面用于存放一些算法需要的信息。算法函数的定义域（类型签名）如下所示：

[.centered]
image::/resource/cp0/cp0-domain.svg[Domain, 60rem]
其中值得注意的是语境 stem:[\gamma]。它被分为以下四种：
...
如果 `e2` 同样是一个 `seq` 结果，则改变其结合性（右结合 stem:[\rightarrow] 左结合），来将处理过的表达式”推到一边”。
...
这里有一个细微的点就是，
如果在处理整个条件表达式时，语境 stem:[\gamma] 具有 *App* stem:[(op, \gamma_1, l_\gamma)] 这样的结构，
说明我们在分析一个被调用的函数，因此我们会取出其中的调用处语境 stem:[\gamma_1] 作为子表达式的语境，以期提供更多信息用于子表达式的优化。
...
赋值时需要查看变量 stem:[x] 是否被使用过，这是通过在环境 stem:[\rho] 中查找是否存在 *ref* 标志而实现的。
如果没有被实际使用，就只需要保留表达式 stem:[e] 中的副作用部分。
...
结构中。
其次我们会生成一个新的语境 *App* stem:[(op, \gamma, l_{y_1})]，在保留当前语境的同时，提供额外的操作数信息。
同时，我们会创建新的语境状态（存储的一部分），标注操作数为 *unvisited* 状态。

处理完被调用者后的操作颇有意思，首先我们判断是否存在 *inlined* 标志，
是的话，则直接用结果 stem:[e_1 '] 替换掉这个 `call` 节点。
...
"visit" 操作有何用途？
由公式所示，它会首先判断该表达式是否已经被优化算法处理过，如果是则返回缓存的结果，否则则使用 stem:[I] 对表达式进行处理，
并通过将结果放入存储 stem:[\sigma] 来缓存。
...
在被用作 *Value* 时，这个优化算法选择 stem:[\alpha]-renaming 后对函数体进行优化，之后组合一个新的优化后的 Lambda 表示。
...
（使用环境 stem:[\rho] 重命名后的）变量已被引用的标志。
否则，我们需要像处理调用表达式时一样，去 "visit" 对应的操作数 stem:[op]，获取优化后的操作数表达式的结果，
...
* 而在折叠 Lambda 表达式时，在完成 stem:[\alpha]-renaming 这一步后，我们需要判断形式参数是否被使用过。若答案为否，且参数也没有被赋值，那只需要依次考虑参数和函数体的副作用，将其变为顺序结构；若形式参数参数未被使用但被赋值过，则操作数仍在只保留副作用的语境下进行优化，并用对 lambda 表达式的调用来构建一个 let 绑定。否则我们必须正常地用 *Value* 语境处理参数，并保留调用表达式的结构。无论何种情况，我们都会设置 *inlined* 标志，以确保上层函数能用新构建的表达式替换掉旧的。
...
* 尝试计数器：对于每次内联操作数的尝试，都维护一个尝试计数器来记录调用 stem:[I] 的次数。如果超出了尝试上限，则终止该次尝试。在内联过程中，如果内嵌的子表达式同样发生了内联，则计数器将被共享。这确保了对于程序每个可能的内联点，都会有一个固定上限。最终，整个程序的总尝试次数是和程序大小线性相关的。
